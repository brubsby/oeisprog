...
def get_random_no_prog_sequence(solved_seqs, max_attempts=5000):
    # Heuristic max
    MAX_A_NUM = 380000
    
    print(f"Searching for unsolved sequence with no OEIS program (max {max_attempts} attempts)...")
    
    for attempt in range(max_attempts):
        num = random.randint(1, MAX_A_NUM)
        a_num = f"A{num:06d}"
        
        if a_num in solved_seqs:
            continue
...
def main():
    ...
    parser.add_argument("--no-prog", action="store_true", help="Pick a random unsolved sequence that also has NO program in OEIS data.")
    args = parser.parse_args()

    choice = None

    if args.no_prog:
        print("Scanning solved sequences...")
        solved_seqs = get_available_sequences('pythonprogs')
        choice = get_random_no_prog_sequence(solved_seqs)
        if not choice:
        ...
        print("Scanning solved sequences...")
        solved_seqs = get_available_sequences('pythonprogs')
        print(f"Found {len(solved_seqs)} solved sequences.")
        
        unsolved = list(all_seqs - solved_seqs)
        print(f"Found {len(unsolved)} sequences with no python program.")
        
        if not unsolved:
            print("No sequences without python program found!")
            return
            
        choice = random.choice(unsolved)
    else:
